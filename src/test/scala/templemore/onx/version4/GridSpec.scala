  ...
  feature("The OnX grid") {
    ...
    scenario("is empty when first created") {
      ...
      for ( row <- 0 to 2; col <- 0 to 2 ) {
         grid.token(Position(row, col)) must be (None)
      }
    }
    ...
    scenario("can have a token added to it") {
      ...
      resultGrid.token(Position(1, 1)) must be (Some(Nought))
    }
    ...
    scenario("can be constructed in a pre-configured state from a string") {
      ...
      grid.token(Position(0, 0)) must be (Some(Nought))
      grid.token(Position(0, 1)) must be (None)
      grid.token(Position(0, 2)) must be (Some(Cross))
      grid.token(Position(1, 0)) must be (Some(Cross))
      grid.token(Position(1, 1)) must be (Some(Nought))
      grid.token(Position(1, 2)) must be (Some(Nought))
      grid.token(Position(2, 0)) must be (None)
      grid.token(Position(2, 1)) must be (None)
      grid.token(Position(2, 2)) must be (Some(Cross))
    }
  }
}
